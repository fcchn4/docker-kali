...
RUN apt install -y whois mlocate bash-completion curl wget vim testssl.sh \
    ...
    udptunnel netcat enum4linux dnsrecon dnsenum dnschef cryptcat powersploit \
    ncrack magicrescue lbd hping3 hashid hash-identifier creddump7 yersinia \
    sleuthkit skipfish spike recon-ng rebind proxychains ptunnel sslstrip \
    sslsplit sparta snmpcheck smbmap set hydra webshells weevely wfuzz whatweb \
    ...
    arp-scan arping binwalk bulk-extractor cewl clusterd commix crunch sslyze \
    bash-completion mlocate theharvester dnsrecon
